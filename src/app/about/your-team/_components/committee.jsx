    ...
    useEffect(() => {
        if (group === "none") {
            const fetchData = async () => {
                try {
                    const response = await fetch(`/website/data/about/your-team/committees/${fileName}.json`);
        ...
        <div className="grid gap-5 grid-cols-1 mx-12 lg:mx-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 pt-4 items-center justify-center">
            {members.map((item, index) => (
                <div key={index} className="w-full h-full flex flex-col">
                    <Image
                        overrideSrc={`/website/images/about/your-team/committees/${fileName}/${item.image}`}
                        alt={item.name}
                        width={1200}
                        height={800}
                        loading='lazy'
